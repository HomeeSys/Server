...
internal class AppendRaportConsumer(ILogger<AppendRaportConsumer> logger,
                                    ...
                                    DevicesService.DevicesServiceClient deviceGRPC) : IConsumer<RaportPending>
{
    public async Task Consume(ConsumeContext<RaportPending> context)
    {
        try
        {
            ...
            var processingStatus = await database.Statuses.FirstOrDefaultAsync(x => x.Name == "Processing");
            ...
            raport.StatusID = processingStatus.ID;
        ...
        {
            logger.LogInformation($"Forwarding to Raport Failed topic!");

            //  Forward to next step
            var failedMessage = new RaportFailed()
            {
                Raport = context.Message.Raport
            };

            await publish.Publish(failedMessage, context.CancellationToken);
        }
    }
